...
contract Exchange {
     ...
     */
    uint8 constant ETH = 1; // id главной сети эфириум
    uint8 constant ETH_KOVAN = 2; // id сети KOVAN
    uint8 constant ETH_RINKEBY = 3; // id сети RINKEBY
    uint8 constant EOS = 4; // id сети EOS
    ...
    /*****************************************************************/
     ...
     /* параметры:
     * _secondBlockchain - id второго блокчейна (например, 5 - BITCOIN)
     * _currencyCount - количество валюты, которую нужно купить (например, 1 биткоин)
     * _priceInWeiForOneUnit - цена за один биткоин (например, 10 эфиров из той сети, в которой мы сейчас)
     */ 
    /**
